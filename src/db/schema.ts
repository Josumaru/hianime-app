...
export const historyManga = pgTable("history_manga", {
	...
	id: bigint({ mode: "number" }).primaryKey().generatedAlwaysAsIdentity({ name: "history_manga_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854, cache: 1 }),
	userId: uuid("user_id"),
	chapter: text(),
	volume: text(),
...
export const historyAnime = pgTable("history_anime", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedAlwaysAsIdentity({ name: "history_anime_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854, cache: 1 }),
	userId: uuid("user_id"),
	cover: text(),
...
export const users = pgTable("users", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).primaryKey().generatedAlwaysAsIdentity({ name: "users_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854, cache: 1 }),
	userId: uuid("user_id"),
	name: text(),
	email: text(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow(),
	animePreferences: text("anime_preferences"),
	themePreferences: text("theme_preferences"),
